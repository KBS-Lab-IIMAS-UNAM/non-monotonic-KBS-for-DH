...
The session start by calling the Prolog interpreter with the instruction `swipl` in a terminal opened in this project folder `non-monotonic-KBS-for-DH/ains_taxonomy/`. The KB engine is loaded with the command:
...
After all the research work is completed, the resulting KB for the case-study in Astronomical Images of New Spain can be seen at `kb_ains_final.txt`, whose digram appears below.
